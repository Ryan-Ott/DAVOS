...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_tensor = torch.where(target_tensor == 15, torch.tensor(2), target_tensor)
        target_tensor = torch.where(target_tensor == 9, torch.tensor(1), target_tensor)
        target_tensor = torch.nn.functional.one_hot(target_tensor.to(torch.int64), num_classes=3).squeeze().permute([2,0,1]).to(torch.float) # 20 classes (VOC)
        '''bcc tensor'''
        # bcc_tensor = torch.nn.functional.interpolate(torch.load(path_item['bcc']), size=(256,256), mode='nearest')
        # bcc_tensor = torch.where(bcc_tensor == 15, torch.tensor(2), bcc_tensor)
        # bcc_tensor = torch.where(bcc_tensor == 9, torch.tensor(1), bcc_tensor)
        # bcc_tensor = torch.nn.functional.one_hot(bcc_tensor.to(torch.int64), num_classes=3).squeeze().permute([2,0,1]).to(torch.float) # 8 classes (see Ozzy's paper)
